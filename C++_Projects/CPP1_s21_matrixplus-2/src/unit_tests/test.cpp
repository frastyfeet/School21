...
TEST(S21MatrixTest, Test_SumMatrix_Error_1) {
  ...
  ASSERT_THROW(matrix_1.SumMatrix(matrix_2), std::invalid_argument);
}
...
TEST(S21MatrixTest, Test_CalcComlements_1) {
  ...
  ASSERT_EQ(matrix_2(0, 0), 0);
  ASSERT_EQ(matrix_2(0, 1), 0);
...
TEST(DeterminantAndCo, InverseMatrix) {
  ...
  for (int i = 0; i < size; i++) {
    for (int j = 0; j < size; j++) {
      EXPECT_EQ(round(matrix_2(i, j)), round(result(i, j)));
    }
  }
}

TEST(S21_MatrixTest, Test_InverseMatrix_Invalid) {
...
TEST(S21MatrixTest, Test_CopyAssignment) {
  const int rows = 3;
  const int cols = 3;
...
TEST(S21MatrixTest, Test_MoveAssignment) {
  const int rows = 3;
  const int cols = 3;
  ...
  EXPECT_EQ(1.0, matrix_2(0, 0));
  EXPECT_EQ(2.0, matrix_2(0, 1));
  EXPECT_EQ(3.0, matrix_2(0, 2));
  EXPECT_EQ(4.0, matrix_2(1, 0));
  EXPECT_EQ(5.0, matrix_2(1, 1));
  EXPECT_EQ(6.0, matrix_2(1, 2));
  EXPECT_EQ(7.0, matrix_2(2, 0));
  EXPECT_EQ(8.0, matrix_2(2, 1));
  EXPECT_EQ(9.0, matrix_2(2, 2));

  EXPECT_EQ(0, matrix_1.GetRows());
  EXPECT_EQ(0, matrix_1.GetColumns());
}
