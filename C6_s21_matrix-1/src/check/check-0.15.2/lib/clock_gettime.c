...
int clock_gettime(clockid_t clk_id CK_ATTRIBUTE_UNUSED, struct timespec *ts) {
  ...
  /*
   * OS X has a function to convert abs time to nano seconds:
   * AbsoluteToNanoseconds However, the function may not be available as we may
   * not have access to CoreServices. Because of this, we convert the abs time
