...
enum { RED, BLUE, PURPLE, YELLOW, BLACK, MAX_TESTS } test_ids;
...
START_TEST(red_test1) { test_executed[RED] = 1; }
END_TEST

START_TEST(blue_test1) { test_executed[BLUE] = 1; }
END_TEST

START_TEST(purple_test1) { test_executed[PURPLE] = 1; }
END_TEST

START_TEST(yellow_test1) { test_executed[YELLOW] = 1; }
END_TEST

START_TEST(black_test1) { test_executed[BLACK] = 1; }
END_TEST
...
static void tag_test_teardown(void) { srunner_free(sr); }
...
Suite *make_tag_suite(void) {
  ...
  tcase_add_unchecked_fixture(exclude_filters, tag_test_setup,
                              tag_test_teardown);
  ...
  tcase_add_unchecked_fixture(include_filters_env, tag_test_setup,
                              tag_test_teardown);
  ...
  tcase_add_unchecked_fixture(exclude_filters_env, tag_test_setup,
                              tag_test_teardown);

  tag_plus_suite_env =
      tcase_create("combining tag filters with suite selection");
  ...
  tcase_add_unchecked_fixture(tag_plus_suite_env, tag_test_setup,
                              tag_test_teardown);
  ...
  tcase_add_unchecked_fixture(tag_plus_case_env, tag_test_setup,
                              tag_test_teardown);
  ...
  tcase_add_unchecked_fixture(strange_filters, tag_test_setup,
                              tag_test_teardown);
