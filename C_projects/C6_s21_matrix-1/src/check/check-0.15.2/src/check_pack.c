...
static size_t get_max_msg_size(void) {
  ...
  if (env)
    value = (size_t)strtoul(env, NULL,
                            10); /* Cast in case size_t != unsigned long. */
  ...
  *buf = ptr = (char *)emalloc(len);
  ...
  if ((size_t)lmsg->line > (size_t)CK_UINT32_MAX)
    eprintf("Value of lmsg->line (%d) too big, max allowed %u\n", __FILE__,
            __LINE__, lmsg->line, CK_UINT32_MAX);
...
RcvMsg *punpack(FILE *fdes) {
  ...
  while (nparse > 0) {
    ...
    if (((ssize_t)nparse - (ssize_t)n) <
        0) /* casting necessary to be able to get proper negative integers */
