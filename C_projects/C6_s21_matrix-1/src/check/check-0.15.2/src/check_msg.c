...
static FILE *get_pipe(void) {
  ...
  eprintf(
      "Unable to report test progress or a failure; was an ck_assert or "
      "ck_abort function called while not running tests?",
      __FILE__, __LINE__);
...
FILE *open_tmp_file(char **name) {
  ...
  *name = NULL;
  ...
  if (file == NULL) {
     ...
     * 4) Use the platform defined temporary directory, or the current
     * directory.
     */
...
static void setup_pipe(void) {
  if (send_file1 == NULL) {
    ...
    if (send_file1 == NULL) {
      eprintf(
          "Unable to create temporary file for communication; may not have "
