...
static void fixture_sub_setup(void) { ck_abort_msg("Test failure in fixture"); }
...
void teardown_fixture(void) { srunner_free(fixture_sr); }
...
START_TEST(test_setup_failure_msg) {
  ...
  char *trm;
  const char *trmexp =
      "check_check_fixture.c:36:S:Fix Sub:unchecked_setup:0: "
...
static void sub_ch_setup_norm(void) { testval_up += 2; }

static void sub_ch_teardown_norm(void) { testval_down += 2; }
...
static void teardown_sub_fail(void) { ck_abort_msg("Failed teardown"); }
...
START_TEST(test_sub_fail) { ck_abort_msg("Should never run"); }
END_TEST

START_TEST(test_sub_pass) { ck_assert_msg(1 == 1, "Always pass"); }
END_TEST
...
START_TEST(test_ch_setup_fail) {
  ...
  ck_assert_msg(strcmp(strstat, "0%: Checks: 1, Failures: 1, Errors: 0") == 0,
  ...
  if (strstr(trm,
             "check_check_fixture.c:150:S:Setup Fail:test_sub_fail:0: "
             "Failed setup") == 0) {
...
START_TEST(test_ch_setup_sig) {
  ...
  ck_assert_msg(strcmp(strstat, "0%: Checks: 1, Failures: 0, Errors: 1") == 0,
  ...
  if (strstr(trm,
             ...
             "(after this point) Received signal 8") == 0) {
...
START_TEST(test_ch_teardown_fail) {
  ...
  if (strstr(trm,
             "check_check_fixture.c:155:S:Teardown Fail:test_sub_pass:0: "
             "Failed teardown") == 0) {
    snprintf(errm, sizeof(errm), "Bad failed checked teardown tr msg (%s)",
             trm);
...
START_TEST(test_ch_teardown_fail_nofork) {
  ...
  if (strstr(trm,
             "check_check_fixture.c:155:S:Teardown Fail No "
             "Fork:test_sub_pass:0: Failed teardown") == 0) {
    snprintf(errm, sizeof(errm), "Bad failed checked teardown tr msg (%s)",
             trm);
...
START_TEST(test_sub_two_teardowns) { testval_down += 2; }
END_TEST
