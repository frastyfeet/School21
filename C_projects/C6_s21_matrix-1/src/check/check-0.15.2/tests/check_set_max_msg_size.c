...
int main(int argc, char *argv[]) {
  ...
  /*
   * Run the test suite. This is intended to trigger the "Message is too long"
   * error. Actual success/failure is determined by examining the output.
   */
  check_set_max_msg_size(32); /* 1st call has no effect since   */
  check_set_max_msg_size(
      strtoul(argv[1], NULL, 10)); /* the 2nd call will override it. */
