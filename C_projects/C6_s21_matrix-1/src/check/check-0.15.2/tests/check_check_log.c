...
START_TEST(test_double_set_log) {
  ...
  ck_assert_msg(
      strcmp(srunner_log_fname(sr), "test_log") == 0,
...
START_TEST(test_set_xml_env) {
  ...
  ck_assert_msg(
      save_set_env("CK_XML_LOG_FILE_NAME", "test_log.xml", &old_val) == 0,
      "Failed to set environment variable");
...
START_TEST(test_double_set_xml) {
  ...
  ck_assert_msg(
      strcmp(srunner_xml_fname(sr), "test_log.xml") == 0,
...
START_TEST(test_set_tap_env) {
  ...
  /* check that setting XML log file via environment variable works */
  ck_assert_msg(
      save_set_env("CK_TAP_LOG_FILE_NAME", "test_log.tap", &old_val) == 0,
      "Failed to set environment variable");
...
START_TEST(test_double_set_tap) {
  ...
  ck_assert_msg(
      strcmp(srunner_tap_fname(sr), "test_log.tap") == 0,
