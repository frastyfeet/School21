...
START_TEST(test_pass) { ck_assert_msg(1 == 1, "Shouldn't see this"); }
END_TEST

START_TEST(test_fail) { ck_abort_msg("Failure"); }
END_TEST
 ...
 */
START_TEST(test_abort) { exit(1); }
END_TEST

START_TEST(test_pass2) { ck_assert_msg(1 == 1, "Shouldn't see this"); }
END_TEST

START_TEST(test_loop) { ck_assert_msg(_i == 1, "Iteration %d failed", _i); }
END_TEST
...
static void print_usage(void) {
  printf(
      "Usage: ex_output (CK_SILENT | CK_MINIMAL | CK_NORMAL | CK_VERBOSE | "
      "CK_ENV");
#if ENABLE_SUBUNIT
  printf(" | CK_SUBUNIT");
#endif
  printf(")\n");
  printf(
      "                 (STDOUT | STDOUT_DUMP | LOG | LOG_STDOUT | TAP | "
      "TAP_STDOUT | XML | XML_STDOUT)\n");
  printf("                 (NORMAL | EXIT_TEST)\n");
  printf(
      "   If CK_ENV is used, the environment variable CK_VERBOSITY can be "
      "set to\n");
  printf(
      "   one of these: silent, minimal, or verbose. If it is not set to "
      "these, or\n");
  printf("   if CK_VERBOSITY is not set, then CK_NORMAL will be used\n");
  printf(
      "   If testing the CK_[LOG|TAP_LOG|XML_LOG]_FILE_NAME env var and "
      "setting it to '-',\n");
  printf(
      "   then use the following mode: CK_SILENT STDOUT [NORMAL|EXIT_TEST].\n");
}

static void run_tests(enum print_output printmode, char *log_type,
